$(function(){
	...
	$('input[name="tipo_busqueda"]').on('change', function(){
		...
		if($(this).val() == 'indicador_region'){
			var filtro_region = $('div#panel-accordion-1').find('div.panelContent');
			var filtro_indicador = $('div#panel-accordion-2').find('div.panelContent');
			$('div#panel-accordion-1').append(filtro_indicador);
			$('div#panel-accordion-2').append(filtro_region);
			filtro_indicador.find('#ind_reg').show();
			filtro_indicador.find('#reg_ind').hide();
			...
			var filtro_indicador = $('div#panel-accordion-1').find('div.panelContent');
	...
	$( "#indicador" ).autocomplete({
		...
		source: function (request, response) {
					var ruta = $('#indicador').data('urlconsulta');
					texto = $('#indicador').val().toLowerCase();
        ...
        open: function(){
        	$('#indicador').removeClass('image_background_loading');
        },
        close: function (event, ui) {
        	if($( "#indicador" ).data('close') == 'false'){
        		$( "#indicador" ).data('close', 'true');
	...
	$(document).on("click", "a[class*='mb-tag-remove']", function(e) {
		...
		if($('input[name="tipo_busqueda"]:checked').val() == 'indicador_region'){
			if($('#lista_tags').find('input[name^="indicador_id"]').length == 0){
				reset_regiones();
	...
	$('#pais, #provincia, #municipio').on('change', function(){
		if(!marca_actulizacion_periodos && ($('input[name="tipo_busqueda"]:checked').val() == 'indicador_region')){
	...
	$('#consulta_indicadores').submit(function() {
		isValid = true;
...
function actualizar_regiones(callback)
{
	if(!actualizando_regiones){
		...
		if($('input[name="tipo_busqueda"]:checked').val() == 'indicador_region')
		{
			...
			if(lista != ''){
				var ruta = $('#indicador').data('consultaregiones');
				...
				$.ajax({
					...
					dataType: 'json',
					success: function (data) {
						update_region($('select#pais'), data.paises, 'No existen paises con informacion para los indicadores seleccionados');
						update_region($('select#provincia'), data.provincias, 'No existen provincias con informacion para los indicadores seleccionados');
						update_region($('select#municipio'), data.municipios, 'No existen municipios con informacion para los indicadores seleccionados');
						fijar_busqueda_multiselect();
...
function limpiar_indicadores_regiones()
{
	$('#lista_tags').empty();
	reset_regiones();
	reset_select_with_list($('select#periodo'), $('#listado_periodos'));
...
function actualizar_periodos(callback)
{
	if(!actualizando_periodos){
		...
		datos = {regiones: zonas,
				 ...
				 _token: $( "#indicador" ).data('token')};
		if((indicadores.length > 0)&&(zonas.length > 0)){
			$.ajax({
				...
				success: function (data) {
					...
					if(data.length > 0)
					{
						...
						$('#periodo').append('<option value="0" disabled> No existe informacion para la combinacion de indicadores/regiones seleccionados </option>');
					}
...
function actualizar_frecuencias(callback)
{
	if(!actualizando_frecuencias){
		...
		datos = {regiones: zonas,
				 ...
				 periodos: periodos,
				 _token: $( "#indicador" ).data('token')};
		if((indicadores.length > 0)&&(zonas.length > 0)&&(periodos.length > 0)){
			$.ajax({
...
function cargar_consulta_previa()
{
	...
	if(consulta.tipo_busqueda == 'indicador_region'){
		$('#busqueda_option2').prop('checked', true);
		$('#busqueda_option2').trigger('change');
		carga_previa_indicadores();
	}
	else{
		carga_previa_regiones();
	}
}
function carga_previa_indicadores()
{
 	//carga de indicadores
	var num_var = $.map(consulta.indicador_id, function(el) { return el }).length;
	$.each(consulta.indicador_id, function(key, value) {
		var tag = $($('#agregar_indicador').html());
		var texto = (consulta.indicador_name[value].length > 60) ? consulta.indicador_name[value].substring(0, 60)+'...' : consulta.indicador_name[value];
		tag.find('span[class="texto"]').html(texto);
		tag.find('span[class="texto"]').prop('title', consulta.indicador_name[value]);
		tag.find('input').prop('name', 'indicador_id['+value+']');
...
function carga_previa_regiones()
{
	...
	$.each(consulta.regiones, function(key, value) {
		...
		if(! --num_regiones){
			...
			if(consulta.tipo_busqueda == 'region_indicador'){
				carga_previa_indicadores();
			}
			else{
				//llamo a actualizar los periodos y cargarlos
